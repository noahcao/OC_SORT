...
namespace ocsort {
    ...
    Eigen::VectorXf speed_direction(const Eigen::VectorXf& bbox1, const Eigen::VectorXf& bbox2) {
    ...
    Eigen::VectorXf convert_x_to_bbox(const Eigen::VectorXf& x) {
    ...
    Eigen::VectorXf k_previous_obs(const std::unordered_map<int, Eigen::VectorXf>& observations_, int cur_age, int k) {
